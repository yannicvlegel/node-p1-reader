...
var electricityReceivedTariff1 = 0;
var electricityReceivedActual = 0.100;
var gasReading = 0;
var gasIncrementValue = 0.100;
...
function EmulateSerialport(path, options, callback) {
    ...
    }, config.emulatorIntervalInSeconds * 1000);
}
 ...
 * @param {number} electricityIncrement - Override the incremental value (every x seconds) of electricity on tariff 1 in kW
 * @param {number} gasOffset - Override the starting value of gas in m3
 * @param {number} gasIncrement - Override the incremental value (every x seconds, set via configuration) of gas in m3
 */
...
function _calculateIncrementedValues() {
    ...
    if ((now - gasLastIncrement) > (config.emulatorGasIntervalInSeconds * 1000)) {
