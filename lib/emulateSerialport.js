...
var electricityReceivedTariff1 = 1000;
var electricityReceivedActual = 100;
var gasReading = 500;
var gasIncrementValue = 50;
var gasLastIncrement = Date.now();
 ...
 * @param {number} electricityOffset - Override the starting value of electricity on tariff 1 in kWh
 * @param {number} electricityIncrement - Override the incremental value (every 10 seconds) of electricity on tariff 1 in kW
 * @param {number} gasOffset - Override the starting value of gas in m3
 * @param {number} gasIncrement - Override the incremental value (every hour) of gas in m3
 * @emits #open - triggered after a short delay to mimic connection setup
 * @emits #data - triggered every 10 seconds
 */
function EmulateSerialport(path, options, callback, electricityOffset, electricityIncrement, gasOffset, gasIncrement) {
    var constructor = this;

    EventEmitter.call(this);

    // Override default values if set
    if (electricityOffset) {
        electricityReceivedTariff1 = electricityOffset;
    }

    if (electricityIncrement) {
        electricityReceivedActual = electricityIncrement;
    }

    if (gasOffset) {
        gasReading = gasOffset;
    }

    if (gasIncrement) {
        gasIncrementValue = gasIncrement;
    }
...
function _calculateIncrementedValues() {
    ...
    if ((now - gasLastIncrement) > config.emulatorGasInterval) {
