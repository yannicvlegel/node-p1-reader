 ...
 * @param {number} electricityStart - Override the starting value of electricity on tariff 1 in kWh
 * @param {number} electricityIncrement - Override the incremental value (every 10 seconds) of electricity on tariff 1 in kW
 * @param {number} gasStart - Override the starting value of gas in m3
 ...
 */
function EmulateSerialport(path, options, callback, electricityStart, electricityIncrement, gasStart, gasIncrement) {
    ...
    if (electricityStart) {
        electricityReceivedTariff1 = electricityStart;
    }
    ...
    if (gasIncrement) {
        gasHourlyIncrement = gasIncrement;
    }
...
function _calculateIncrementedValues() {
    ...
    output.gasReading = gasReading + gasHourlyIncrement;

    // Store the cumulative values globally for the next increment
